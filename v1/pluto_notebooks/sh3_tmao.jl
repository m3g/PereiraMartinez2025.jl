...
# ╔═╡ be0afe79-8483-4861-8a3e-2e2a6c944fe6
using CSV
...
# ╔═╡ 1f833f9f-43af-4044-b939-fdb3de24176d
using ColorSchemes
...
md"# SH3 transfer free energy to TMAO"
...
- Native state: Most representative structure of the $N^{S0}_{80}$ basin.
- First denatured state: Most representative structure of the $U^{S1}_{46}$ basin.
- Second denatured state: Most representative structure of the $U^{S12}_{1.0}$ basin.

From here on, these states will be referred to as **N**, **U1**, and **U2**.
"""

# ╔═╡ fb3f40bf-4b4b-4992-9eb7-667d8b12fe64
md"The molalities of TMAO in the bulk phase of each simulated concentration are:"

# ╔═╡ b2529e4f-a312-4adc-adf2-3f1f70a7b35f
md"### Simulated preferential interactions:"

# ╔═╡ bb81d137-effe-4107-a61b-fd529cff8421
md"The preferential interaction parameters of SH3 in TMAO, for the N, U1, and U2 states, are (converted from mol/mol to g/g):"

# ╔═╡ 813427fb-7d88-44f3-b118-2d5b94031a50
∂g₃∂g₂ = DataFrame(
	"N" => [ -0.4472249833194395, -0.922583206397835, -1.4163864393453083, -1.8799580331405132, -2.37749310164275, -3.390462347824896, -4.691678842892974 ] / (M2/M3),
	"U1" => [ -0.6159997409328666, -1.0720040799156296, -1.5421758466275783, -2.178883097467213, -2.698511442318831, -4.031083826957941, -5.058291395707879 ] / (M2/M3),
	"U2" => [ -0.7844982665365943, -1.6043591305951068, -2.3243926785825346, -2.900986873271736, -3.8835013980386734, -5.393531177920885, -6.714690401220578 ] / (M2/M3),
)
...
md"""
The results imply that the denatured states are slightly less favorably transferred to a 0.5 mol/L aqueous TMAO solution than the native state. 
The difference increases with increasing TMAO concentration. At higher TMAO concentrations the integrals associated with the denatured states become progressively more positive, thus the denatured states are destabilized relative to the native state upon transfer to a TMAO solution.
"""
...
md"Experimental data for TMAO aqueous solutions (at 25$^\circ$C):"
...
begin
	...
	c3 = uconvert.(u"mol/L", tmao_data[!,:m₃] .* tmao_data[!,:ρ₀])
	p1 = scatter(MolSimStyle, 
		c3, tmao_data[!,:ρ₀],
		label=:none,
	    xlabel=L"c_3", ylabel=L"\rho_0", color=:blue,
	)
	density_fit = fitquadratic(c3, tmao_data[!,:ρ₀])
...
md"For the N and U states, the obtained bulk concentrations of TMAO are obtained from the simulations in mol/L:"

# ╔═╡ 14f768ba-30e1-4fbb-ba80-70065873a830
c = DataFrame(
    "N" => [ 0.10168138557624935, 0.21246537665408965, 0.3128632398647313, 0.42119626599709525, 0.5264188451319083, 0.7900113942209422, 1.0619609909023096 ]u"mol/L",
    "U1" => [ 0.10533873343088904, 0.20956631985612292, 0.3149834986079143, 0.4237250638997249, 0.5265935927958126, 0.7931601287343764, 1.0519645331208074  ]u"mol/L",
    "U2" => [ 0.10626362323352392, 0.21275674665435967, 0.3191249540649642, 0.41632850537888244, 0.5280721577058654, 0.7915367754441315, 1.0483501812282612  ]u"mol/L",
)
...
begin
	p2 = plot()
	states = ["N", "U1", "U2"]
	labels = [ L"{\mathrm{N^{S0}_{80}}}", L"{\mathrm{U^{S1}_{46}}}", L"{\mathrm{U^{S12}_{1.0}}}" ]
	colors = [ get(ColorSchemes.okabe_ito,(13)/13), get(ColorSchemes.okabe_ito,(12)/13), get(ColorSchemes.okabe_ito,(1)/13) ]
	...
	#savefig("SH3_TMAO_TFE.pdf")
end

# ╔═╡ 4d8bfae0-2eda-462b-a9ec-c0082097305d
begin
	p3 = scatter(MolSimStyle,
		simulated_m₃[!,"N"], ∂g₃∂g₂[!,"N"], label=L"{\mathrm{N^{S0}_{80}}}",
		xlabel=L"m_3", ylabel=L"\partial g_2\partial g_3 \mathrm{(g/g)}",
		color=colors[1]
    )
	scatter!(p3,
		simulated_m₃[!,"U1"], ∂g₃∂g₂[!,"U1"], label=L"{\mathrm{U^{S1}_{46}}}",
		color=colors[2]
	)
	scatter!(p3,
		simulated_m₃[!,"U2"], ∂g₃∂g₂[!,"U2"], label=L"{\mathrm{U^{S12}_{1.0}}}",
		color=colors[3], dpi=300
	)
	plot!(p3, size=(400,300))
	#savefig("SH3_TMAO_Γ.pdf")
end

# ╔═╡ 503c33d8-cd92-4651-95f8-5be1296a7f57
let
...
#CSV.write("./sh3_tmao_delta_mu_values.csv", df)
...
# ╔═╡ 2499a71b-0a75-4edb-96b4-fee1e531a7a8
 VERSION
...
git-tree-sha1 = "48a249fcd3cc9032191d08216e1acc2d5ca5a321"
uuid = "74334e00-59ce-546d-b517-81f3b7e1d491"
version = "4.4.0+1"
...
git-tree-sha1 = "ddc47124cc58e550c8b89a54afbaba7556c1e796"
...
[[deps.ProteinSecondaryStructures]]
deps = ["DSSP_jll", "STRIDE_jll", "TestItems"]
git-tree-sha1 = "da7a6710aff45dfab7c4b7da921ef49aa1605364"
uuid = "c298f58b-271f-48cb-96ac-5442ca7c67d9"
version = "2.0.3"
...
[[deps.STRIDE_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "d5f4a36ba095f235070ad651434e74d4de3d63fe"
uuid = "850473c1-9ef0-5df9-a957-757f4cde8b8b"
version = "1.0.0+0"
